...
description: "Comprehensive guide to handling errors and exceptions in PinePay API integration"
---

# Error Handling Guide

Proper error handling is crucial for building reliable applications with PinePay API. This guide covers common errors, best practices, and implementation strategies.

## HTTP Status Codes

PinePay API uses standard HTTP status codes to indicate the success or failure of requests:
...
class PinePayError(Exception):
    """Base exception for PinePay errors"""
    pass

class ValidationError(PinePayError):
    def __init__(self, message, errors=None):
        super().__init__(message)
        self.errors = errors

class AuthenticationError(PinePayError):
    pass

class RateLimitError(PinePayError):
    def __init__(self, message, retry_after=None):
        super().__init__(message)
        self.retry_after = retry_after

class ServerError(PinePayError):
    pass

class NetworkError(PinePayError):
    pass

class APIError(PinePayError):
...
```php PHP
<?php
class PinePayError extends Exception {
    protected $errors;
...
class ValidationError extends PinePayError {}
class AuthenticationError extends PinePayError {}
class RateLimitError extends PinePayError {
...
class ServerError extends PinePayError {}
class NetworkError extends PinePayError {}
...
<?php
// Configure logging
$logger = new Monolog\Logger('pinepay');
...
class PinePayClient {
    ...
    public function __construct($apiKey, $baseUrl = 'https://api.pinepay.com') {
        $this->apiKey = $apiKey;
        $this->baseUrl = $baseUrl;
        $this->logger = new Monolog\Logger('pinepay');
    }
