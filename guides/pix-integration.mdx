...
description: "Complete guide to integrating PIX payments with braix API"
---

# PIX Integration Guide

This comprehensive guide will walk you through integrating PIX payments with braix API, from basic setup to advanced features like webhooks and refunds.

## Overview

PIX (Sistema de Pagamentos Instantâneos) is Brazil's instant payment system that allows real-time transfers 24/7. braix provides a unified API to integrate with multiple PIX gateways, simplifying the complexity of different provider implementations.

## Prerequisites

Before starting your integration, ensure you have:

- A braix account with API credentials
...
class braixClient {
  constructor(apiKey, baseUrl = 'https://api.braix.com') {
    this.apiKey = apiKey;
...
// Usage
const client = new braixClient(process.env.braix_API_KEY);
...
class braixClient:
    def __init__(self, api_key, base_url='https://api.braix.com'):
...
# Usage
client = braixClient(os.getenv('braix_API_KEY'))
...
class braixClient {
    ...
    public function __construct($apiKey, $baseUrl = 'https://api.braix.com') {
        $this->apiKey = $apiKey;
        $this->baseUrl = $baseUrl;
    }
...
// Usage
$client = new braixClient($_ENV['braix_API_KEY']);
