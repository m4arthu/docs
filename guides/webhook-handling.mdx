...
All braix webhooks include a signature for verification. Always verify the signature to ensure the webhook is authentic:
...
app.post('/webhooks/braix', async (req, res) => {
  const deliveryId = req.headers['x-braix-delivery'];
  const { type, data } = req.body;
...
@app.route('/webhooks/braix', methods=['POST'])
def handle_webhook():
    delivery_id = request.headers.get('X-braix-Delivery')
    data = request.get_json()
    webhook_type = data.get('type')
...
<?php
// webhook.php
$deliveryId = $_SERVER['HTTP_X_braix_DELIVERY'] ?? '';
$input = file_get_contents('php://input');
$data = json_decode($input, true);
