...
description: "Learn how to authenticate with PinePay API using OAuth 2.0 client credentials flow"
---

# Authentication

PinePay API uses OAuth 2.0 client credentials flow for secure access to all endpoints. This guide explains how to authenticate your requests and manage your API credentials.

## Client Credentials

Your client credentials (client ID and client secret) are used to obtain access tokens. You can find your credentials in the [PinePay Dashboard](https://dashboard.pinepay.com) under the "API Keys" section.
...
To authenticate with PinePay API, you need to:
...
const getAccessToken = async () => {
  const clientId = process.env.PINEPAY_CLIENT_ID;
  const clientSecret = process.env.PINEPAY_CLIENT_SECRET;
  const credentials = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
  
  const response = await axios.post('https://api.pinepay.com/auth/token', {}, {
    headers: {
...
def get_access_token():
    client_id = os.getenv('PINEPAY_CLIENT_ID')
    client_secret = os.getenv('PINEPAY_CLIENT_SECRET')
    credentials = base64.b64encode(f"{client_id}:{client_secret}".encode()).decode()
    
    response = requests.post('https://api.pinepay.com/auth/token', 
...
function getAccessToken() {
    $clientId = $_ENV['PINEPAY_CLIENT_ID'];
    $clientSecret = $_ENV['PINEPAY_CLIENT_SECRET'];
    ...
    $response = file_get_contents('https://api.pinepay.com/auth/token', false, $context);
...
```curl cURL
# Step 1: Get access token
curl -X POST "https://api.pinepay.com/auth/token" \
...
```bash .env
PINEPAY_CLIENT_ID=your_client_id_here
PINEPAY_CLIENT_SECRET=your_client_secret_here
